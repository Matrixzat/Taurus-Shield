...
set_var() {
    ...
    if [ -z "$java_file" ] || [ ! -f "$java_file" ]; then
        error "MainActivity.java not found"
    fi
    ...
    # Добавляем кавычки если значение не true/false и не число
...
apply_config() {
    ...
    echo "[DEBUG] Parsing merged config file: $config_file" >&2
    ...
    while IFS='=' read -r key value || [ -n "$key" ]; do
        ...
        # Debug icon specifically
        if [ "$key" = "icon" ]; then
            echo "[DEBUG] Found icon key, raw value: '$value'" >&2
        fi
...
apk() {
    ...
    if [ -f "app/build/outputs/apk/release/app-release.apk" ]; then
        ...
        # Convert package to directory path for finding source files
        local package_path=$(echo "$full_package" | tr '.' '/')
        ...
        echo -e "Package: ${BLUE}$full_package${NC}"
        echo -e "App name: ${BLUE}$(grep -o 'app_name">[^<]*' app/src/main/res/values/strings.xml | cut -d'>' -f2)${NC}"
        echo -e "URL: ${BLUE}$(grep 'String mainURL' app/src/main/java/$package_path/*.java 2>/dev/null | head -1 | cut -d'"' -f2)${NC}"
        echo -e "${BOLD}----------------${NC}"
    else
        error "Build failed"
    fi
...
chid() {
    ...
    # Get the current full package name from build.gradle
    old_package=$(grep -oP 'applicationId "\K[^"]+' app/build.gradle)
    
    # Check if input contains dots (full package name like com.company.app)
    if echo "$raw_id" | grep -q '\.'; then
        # Full package name provided - sanitize each segment (keep dots)
        safe_id=$(echo "$raw_id" | tr 'A-Z' 'a-z' | sed 's/[^a-z0-9.]//g' | sed 's/\.\.*/./g' | sed 's/^\.\|\.$//g')
        new_package="$safe_id"
    else
        # Simple ID provided - wrap in com.{id}.webtoapk
        safe_id=$(echo "$raw_id" | tr -cd '[:alnum:]' | tr 'A-Z' 'a-z')
        new_package="com.${safe_id}.webtoapk"
    fi

    if [ -z "$safe_id" ]; then
        error "Invalid application ID. Must contain letters or numbers"
    fi

    info "Old package: $old_package"
    info "New package: $new_package"

    if [ "$old_package" = "$new_package" ]; then
        log "Package name unchanged"
        return 0
    fi

    # Update package name in all relevant files
    local escaped_old=$(echo "$old_package" | sed 's/\./\\./g')
    info "Replacing $old_package with $new_package in all files..."
    ...
    local old_dir=$(echo "$old_package" | tr '.' '/')
    local new_dir=$(echo "$new_package" | tr '.' '/')
    
    # Move Java source files to new package directory
    if [ -d "app/src/main/java/$old_dir" ]; then
        info "Moving source files from $old_dir to $new_dir..."
        mkdir -p "app/src/main/java/$new_dir"
        mv "app/src/main/java/$old_dir"/* "app/src/main/java/$new_dir/" 2>&1
        
        # Clean up old empty directories - but only if they don't overlap with new path
        local old_root=$(echo "$old_package" | cut -d. -f1)
        local new_root=$(echo "$new_package" | cut -d. -f1)
        if [ "$old_root" != "$new_root" ]; then
            rm -rf "app/src/main/java/$old_root" 2>/dev/null
        else
            # Same root - just remove the old leaf directories that are now empty
            rmdir "app/src/main/java/$old_dir" 2>/dev/null
            # Try to clean up empty parent dirs (but not the shared root)
            local parent_dir=$(dirname "$old_dir")
            while [ "$parent_dir" != "." ] && [ "$parent_dir" != "$new_root" ]; do
                rmdir "app/src/main/java/$parent_dir" 2>/dev/null || break
                parent_dir=$(dirname "$parent_dir")
            done
        fi
        log "Source files moved successfully"
    fi

    # Update appname variable for other functions
    appname=$(echo "$new_package" | rev | cut -d. -f1 | rev)
    log "Package name changed to: $new_package"
}
...
set_icon() {
    ...
    local foreground_file="app/src/main/res/drawable/ic_launcher_foreground.png"
    
    echo "[DEBUG] set_icon called with: '$icon_path'" >&2
    echo "[DEBUG] CONFIG_DIR: '${CONFIG_DIR:-}'" >&2
    
    # If no icon provided, use default
    if [ -z "$icon_path" ]; then
        icon_path="$default_icon"
        echo "[DEBUG] Using default icon: $icon_path" >&2
    fi

    # If icon_path is not absolute, prepend CONFIG_DIR
    if [ -n "${CONFIG_DIR:-}" ] && [[ "$icon_path" != /* ]]; then
        icon_path="$CONFIG_DIR/$icon_path"
        echo "[DEBUG] Resolved path: $icon_path" >&2
    fi

    # Check if file exists
    echo "[DEBUG] Checking if icon exists at: $icon_path" >&2
    if [ -f "$icon_path" ]; then
        echo "[DEBUG] Icon file EXISTS" >&2
    else
        echo "[DEBUG] Icon file NOT FOUND" >&2
    fi
    ...
    mkdir -p "$(dirname "$foreground_file")"
    ...
    # Copy icon to legacy mipmap location
    try "cp \"$icon_path\" \"$dest_file\""
    
    # Copy icon to adaptive icon foreground location
    try "cp \"$icon_path\" \"$foreground_file\""
    
    log "Icon updated successfully (legacy + adaptive)"
}
...
# Check Java version and update JAVA_HOME if needed (accepts Java 17+)
check_and_find_java() {
    ...
    if [ -n "${JAVA_HOME:-}" ] && [ -x "$JAVA_HOME/bin/java" ]; then
        ...
        if [ "$version" -ge 17 ] 2>/dev/null; then
            info "Using system JAVA_HOME: $JAVA_HOME (Java $version)"
            export PATH="$JAVA_HOME/bin:$PATH"
            return 0
        else
            warn "Current JAVA_HOME points to incompatible version: $version (need 17+)"
        fi
    fi
    ...
    # Finally check /usr/lib/jvm for Java 17+
    if [ -d "/usr/lib/jvm" ]; then
        while IFS= read -r java_path; do
            if [ -x "$java_path/bin/java" ]; then
                ...
                if [ "$version" -ge 17 ] 2>/dev/null; then
                    info "Found system Java $version: $java_path"
...
full_package=$(grep -oP 'applicationId "\K[^"]+' app/build.gradle)

# For appname, use the last segment of the package (or full if no dots)
if echo "$full_package" | grep -q '\.'; then
    appname=$(echo "$full_package" | rev | cut -d. -f1 | rev)
else
    appname="$full_package"
fi

# Clean up appname - remove _webtoapk suffix if present
appname=$(echo "$appname" | sed -E 's/_*webtoapk$//')

# If appname is now empty (e.g., package was com.xxx.webtoapk), use second-to-last segment
if [ -z "$appname" ] && echo "$full_package" | grep -q '\.'; then
    appname=$(echo "$full_package" | rev | cut -d. -f2 | rev)
fi
...
if ! check_and_find_java; then
    ...
    # In non-interactive mode (e.g., subprocess), skip the prompt and error out
    if [ ! -t 0 ]; then
        error "Java 17+ is required. Please install JDK 17 or newer."
    fi
...
# Final verification - accept Java 17 or newer
java_version=$(java -version 2>&1 | head -n 1 | cut -d'"' -f2 | cut -d'.' -f1)
if [ "$java_version" -lt 17 ] 2>/dev/null; then
    error "Wrong Java version: $java_version. Java 17 or newer is required"
fi
