...
class SslUnpinService : Service() {
    ...
    private fun patchSingleDex(dex: ByteArray, label: String): Pair<ByteArray, Int> {
        ...
        return try {
            ...
            // touched them.  MT Manager's "Reformat code" fixes this by rebuilding the
            // DEX from smali (recalculating all metadata).  We replicate the same fix:
            // clamp outs_size down to registers_size for every method in the DEX.
            // This is safe: outs_size is a stack-frame-size hint; ART uses the actual
            // invoke instruction for argument dispatch at runtime.
            // ── outs > regs repair — tracked outside block so early-return sees it ──
            var outsRepaired = 0
            run {
                fun u16r(off: Int) =
                ...
                fun u32r(off: Int) =
                    ...
                    ((data[off + 2].toInt() and 0xFF) shl 16) or
                    ((data[off + 3].toInt() and 0xFF) shl 24)
                fun ulebR(startOff: Int): Pair<Int, Int> {
                    var r = 0; var s = 0; var o = startOff
                    while (true) {
                        val b = data[o++].toInt() and 0xFF
                        r = r or ((b and 0x7F) shl s); s += 7
                        if (b and 0x80 == 0) break
                    }
                    return r to o
                }

                val classDefsSize = u32r(0x60)
                val classDefsOff  = u32r(0x64)

                for (ci in 0 until classDefsSize) {
                    val cdBase = classDefsOff + ci * 32
                    val classDataOff = u32r(cdBase + 24)
                    if (classDataOff == 0) continue

                    var p = classDataOff
                    fun nextU(): Int { val (v, nx) = ulebR(p); p = nx; return v }

                    val sfSz = nextU(); val ifSz = nextU()
                    val dmSz = nextU(); val vmSz = nextU()
                    repeat(sfSz + ifSz) { nextU(); nextU() }   // skip fields

                    repeat(dmSz + vmSz) {
                        nextU()                                 // method_idx_diff
                        nextU()                                 // access_flags
                        val co = nextU()                        // code_off
                        if (co == 0 || co + 6 > data.size) return@repeat
                        val regs = u16r(co)
                        val outs = u16r(co + 4)
                        if (outs > regs) {
                ...
                if (outsRepaired > 0) log("  Repaired $outsRepaired code_item(s) with outs > regs (pre-existing)")
            }
